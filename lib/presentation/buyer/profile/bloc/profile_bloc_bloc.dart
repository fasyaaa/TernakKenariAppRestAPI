...
import 'package:carnary/data/model/request/buyer/buyer_profile_request_model.dart';
...
class ProfileBuyerBloc extends Bloc<ProfileBuyerEvent, ProfileBuyerState> {
  final ProfileBuyerRepository profileBuyerRepository;
    ProfileBuyerBloc({required this.profileBuyerRepository}) : super(ProfileBuyerInitial()){on<AddProfileBuyerEvent>(_addProfileBuyer); on<GetProfileBuyerEvent>(_getProfileBuyer);}
    ...
    Future<void> _addProfileBuyer(
      ...
      result.fold((error) => emit(ProfileBuyerAddError(message: error)), (profile) {
        emit(ProfileBuyerAdded(profile: profile));
      });
    }

    Future<void> _getProfileBuyer(GetProfileBuyerEvent event, Emitter<ProfileBuyerState> emit) async{
      emit(ProfileBuyerLoading());
      final result = await profileBuyerRepository.getProfileBuyer();
      result.fold((error) => emit(ProfileBuyerError(message: error)), (profile) => emit(ProfileBuyerLoaded(profile: profile)));
    }
}
